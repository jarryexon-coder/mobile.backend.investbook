...
const getBackendCache = async () => {
    try {
        ...
        console.log('⚠️ Backend cache returned empty data');
        return null;
    } catch (error) {
        console.log('⚠️ Backend cache not available:', error.message);
        // If 404, the endpoint doesn't exist yet - that's okay, use AsyncStorage
        if (error.response?.status === 404) {
            console.log('📌 Cache endpoint not found, using AsyncStorage only');
        }
        return null;
    }
};
